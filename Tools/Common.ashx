...
<%@ WebHandler Language="C#" Class="Freifunk.Webpages.Common" %>
...
[assembly: AssemblyProduct("Freifunk Tools")]
[assembly: AssemblyCopyright("cc-by-nc Matthias Schäfer freifunk-tox gmx de")]
...
[assembly: Guid("0b0192cf-6e5d-49a4-95d3-97ac8e04a6ff")]
...
{
   internal class Common : IHttpHandler
   {
      public bool IsReusable
      {
         // nicht implementiert
         get { throw new NotImplementedException("Diese Webseite ist nicht für den direkten Aufruf vorgesehen."); }
      }
      ...
      public void ProcessRequest(HttpContext context)
      {
         ...
         // sonst nicht implementiert
         throw new NotImplementedException("Diese Webseite ist nicht für den direkten Aufruf vorgesehen.");
      }
   }
}
...
namespace Freifunk
{
   internal class EtxGraph
   {
      ...
      // gibt zurück, ob ein Knoten Teil eines VPN ist
      public bool ExistsTunneledNode(string ip)
      {
   ...
   internal class NetTools
   {
      ...
      public static WebResponse GetWebResponse(string uri, int timeout)
      {
         ...
         try
         {
            // Anfrage erstellen, Antwort-Timeout und ggf. Kodierung festlegen
            WebRequest Request;
            (Request = WebRequest.Create(uri)).Timeout = timeout;
            if (Request is System.Net.HttpWebRequest)
               ((System.Net.HttpWebRequest)Request).Headers.Add(HttpRequestHeader.AcceptEncoding, "gzip, identity");
            // Antwort abrufen und Timeout festlegen, wenn anwendbar
            WebResponse Response;
            if ((ResponseStream = (Response = Request.GetResponse()).GetResponseStream()).CanTimeout)
               ResponseStream.ReadTimeout = timeout;
            return Response;
         }
         catch (WebException ex)
         {
            // C# unterstützt keine Filter; Timeout festlegen, wenn Antwort vorhanden und anwendbar
            if (ex.Response != null && (ResponseStream = ex.Response.GetResponseStream()).CanTimeout)
               ResponseStream.ReadTimeout = timeout;
            throw;
         }
      }
   ...
   internal class EncodingTools
   {
      ...
      public StreamReader GetReader(WebResponse webResponse, string settingsPrefix)
      {
         ...
         bool CompMode = HttpWebResponse != null && HttpWebResponse.ContentEncoding == "gzip";
         ...
         return new StreamReader(CompMode ? new System.IO.Compression.GZipStream(webResponse.GetResponseStream(), System.IO.Compression.CompressionMode.Decompress) : webResponse.GetResponseStream(), Enc ?? new ASCIIEncoding(), AutoMode);
      }
   }
}
